...
- 未提供 `workspace` 时，先要求用户提供；不要把示例路径当默认值，也不要自行扫描磁盘猜测工作副本。
- 未提供 `time_range` 时，先询问使用自定义范围，还是默认业务日窗口。
- 如果 Agent 判断当前是沙箱、CI 或受限自动化用户，未提供 `svn_config_dir` 时，先要求用户提供或创建独立 SVN config 目录。
- 完整代码审查不可裁剪：第一阶段成功且 revision 列表非空后，必须依次执行 `svn update`、Maven 构建、实体生成、逐 revision 拉 diff、生成审查日志。
- 缺少 Maven 或实体生成配置时，先要求用户补齐；不要退化成“只基于日志和 diff 的审查”。
- 只有用户明确要求“只看 diff”“跳过构建”或“跳过实体生成”时，才允许裁剪完整审查流程，并必须在审查日志中说明结论受限。
